...
import com.quantum_prof.eurpaparkwaittimes.data.network.ApiService
...
class WaitTimeRepository @Inject constructor(
    ...
    suspend fun getEuropaparkWaitTimes(forceRefresh: Boolean = false): Result<WaitTimeResult> {
    ...
    suspend fun getCrowdLevel(): Result<CrowdLevel> {
        return withContext(Dispatchers.IO) {
            try {
                val crowdLevel = apiService.getCrowdLevel()
                Result.success(crowdLevel)
            } catch (e: Exception) {
                Result.failure(e)
            }
        }
